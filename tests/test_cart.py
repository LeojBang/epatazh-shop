...
async def _make_product_with_variant(
    db_session, price="3000.00", sale_price=None, stock=10, size="M"
):
...
class TestAddToCart:
    ...
    async def test_add_existing_variant(self, fake_redis, db_session):
        ...
        result = await service.add_to_cart(
            fake_redis, db_session, "user1", str(variant.id), 2
        )
    ...
    async def test_add_nonexistent_variant(self, fake_redis, db_session):
        ...
        result = await service.add_to_cart(
            fake_redis, db_session, "user1", "00000000-0000-0000-0000-000000000000", 1
        )
    ...
    async def test_add_out_of_stock(self, fake_redis, db_session):
        ...
        result = await service.add_to_cart(
            fake_redis, db_session, "user1", str(variant.id), 1
        )
        assert result["ok"] is False
...
class TestCartWithProducts:
    ...
    async def test_total_without_discount(self, fake_redis, db_session):
        ...
        variant = await _make_product_with_variant(
            db_session, price="3000.00", stock=10
        )
        await service.add_to_cart(fake_redis, db_session, "user1", str(variant.id), 2)

        items, total = await service.get_cart_with_products(
            fake_redis, db_session, "user1"
        )
    ...
    async def test_total_with_discount(self, fake_redis, db_session):
        ...
        variant = await _make_product_with_variant(
            db_session, price="3000.00", sale_price="2400.00", stock=10
        )
        await service.add_to_cart(fake_redis, db_session, "user1", str(variant.id), 2)
