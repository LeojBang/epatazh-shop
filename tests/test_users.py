...
class TestCreateUser:
    ...
    async def test_get_user_by_email(self, db_session):
        ...
        user_in = UserCreate(email="find@example.com", password="secret123", full_name="Поиск")
...
class TestAuthentication:
    ...
    async def test_correct_credentials(self, db_session):
        ...
        user_in = UserCreate(email="auth@example.com", password="secret123", full_name="Auth")
        await service.create_user(db_session, user_in)

        user = await service.authenticate_user(db_session, "auth@example.com", "secret123")
        assert user is not None
        assert user.email == "auth@example.com"

    async def test_wrong_password(self, db_session):
        """Неверный пароль возвращает None."""
        user_in = UserCreate(email="auth2@example.com", password="secret123", full_name="Auth")
        await service.create_user(db_session, user_in)

        user = await service.authenticate_user(db_session, "auth2@example.com", "wrongpass")
        assert user is None

    async def test_nonexistent_email(self, db_session):
        """Несуществующий email возвращает None."""
        user = await service.authenticate_user(db_session, "ghost@example.com", "secret123")
        assert user is None
...
class TestChangePassword:
    ...
    async def test_correct_change(self, db_session):
        ...
        user_in = UserCreate(email="pwd@example.com", password="oldpass123", full_name="Pwd")
        user = await service.create_user(db_session, user_in)

        error = await service.change_password(db_session, user, "oldpass123", "newpass456")
        assert error is None  # нет ошибки = успех

        # старый пароль больше не подходит, новый подходит
        assert await service.authenticate_user(db_session, "pwd@example.com", "newpass456") is not None
        assert await service.authenticate_user(db_session, "pwd@example.com", "oldpass123") is None

    async def test_wrong_current_password(self, db_session):
        """С неверным текущим паролем смена отклоняется."""
        user_in = UserCreate(email="pwd2@example.com", password="oldpass123", full_name="Pwd")
        user = await service.create_user(db_session, user_in)

        error = await service.change_password(db_session, user, "wrongold", "newpass456")
    ...
    async def test_too_short_new_password(self, db_session):
        ...
        user_in = UserCreate(email="pwd3@example.com", password="oldpass123", full_name="Pwd")
        user = await service.create_user(db_session, user_in)
...
class TestUpdateProfile:
    ...
    async def test_update_basic_fields(self, db_session):
        ...
        user_in = UserCreate(email="prof@example.com", password="secret123", full_name="Старое")
        user = await service.create_user(db_session, user_in)

        updated, error = await service.update_profile(
            db_session, user,
            full_name="Новое имя", email="prof@example.com",
    ...
    async def test_email_taken_by_another(self, db_session):
        ...
        await service.create_user(db_session, UserCreate(email="first@example.com", password="secret123", full_name="A"))
        user_b = await service.create_user(db_session, UserCreate(email="second@example.com", password="secret123", full_name="B"))

        # пытаемся дать второму email первого
        updated, error = await service.update_profile(
            db_session, user_b,
            full_name="B", email="first@example.com",
